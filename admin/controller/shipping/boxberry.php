...
class Boxberry extends \Opencart\System\Engine\Controller
{
    ...
    public function index()
    {
        ...
        $data = array_merge($data, $load->language('extension/boxberry/shipping/boxberry'));
        ...
        $data['breadcrumbs'] = [
            ...
            [
                ...
                'href' => $url->link("extension/boxberry/shipping/boxberry", $tokenName . '=' . $data['user_token'] . '&type=shipping', true)
            ]
        ];
        $data['action'] = $url->link("extension/boxberry/shipping/boxberry", $tokenName . '=' . $data['user_token'] . '&type=shipping', true);
        ...
        $this->response->setOutput($load->view("extension/boxberry/shipping/boxberry", $data));
    }
    ...
    public function install()
    {
       ...
       // Изменяем способ выдачи прав
       // $this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'sale/boxberry');
       $this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/' . $extension . '/sale/' . $code);
       $this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/' . $extension . '/sale/' . $code);
        /** Дополнения после изучения работы контроллера shipping  */
        ...
        $_event_boxberry_order_status_edit = [
                    'description'	=> 'catalog boxberry_order_status_edit event after',
                    'code'	=> 'boxberry_order_status_edit',
                    'trigger'	=> 'catalog/model/checkout/order/addOrderHistory/after',
                    'action'	=> 'extension/boxberry/shipping/boxberry|addOrderHistory',
                    'status'    => 1,
                    'sort_order'=> 1
        ];

        if($result = $this->model_setting_event->getEventByCode('boxberry_order_status_edit')) {
            $this->model_setting_event->deleteEventByCode('boxberry_order_status_edit');
        }
        $this->model_setting_event->addEvent($_event_boxberry_order_status_edit);

        //$this->model_setting_event->deleteEventByCode('boxberry_add_scripts');
        $_event_boxberry_add_scripts = [
            'description'	=> 'catalog boxberry_add_scripts event before',
            'code'	=> 'boxberry_add_scripts',
            'trigger'	=> 'catalog/model/checkout/order/addOrderHistory/after',
            'action'	=> 'extension/boxberry/shipping/boxberry|addScripts',
            'status'    => 1,
            'sort_order'=> 0
        ];

        if($result = $this->model_setting_event->getEventByCode('boxberry_add_scripts')) {
            $this->model_setting_event->deleteEventByCode('boxberry_add_scripts');
        }
        $this->model_setting_event->addEvent($_event_boxberry_add_scripts);
    ...
    public function uninstall()
    {
        ...
        // Кажется, что здесь нужно менять
...
class ControllerExtensionShippingBoxberry extends Boxberry
{
}

class ControllerShippingBoxberry extends Boxberry
{
}
